...
def bar(done, total, width=20):
    ...
    return "[" + "#" * filled + "-" * (width - filled) + f"] {pct:3d}% ({done}/{total})"
...
def cmd_render(data):
    ...
    for typ, groups in types.items():
        ...
        if not tt:
            continue
        td = [t for t in tt if t.get("status") == "done"]
        out.append(f"- **{typ}** `{bar(len(td), len(tt), 16)}`")
        for g in groups:
            gt = [t for t in tt if t.get("group") == g]
            if not gt:
                continue
            gd = [t for t in gt if t.get("status") == "done"]
            out.append(f"    - {g}: `{bar(len(gd), len(gt), 12)}`")
